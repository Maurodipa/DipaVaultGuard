...
// ===== NUOVA FUNZIONE PER DERIVARE CHIAVE CON SECONDO FATTORE =====
// Utile per combinare password + PRF secret o TOTP secret per derivare la chiave di vault
export async function deriveVaultKeyWithSecondFactor(password, secondFactorSecret, salt = null) {
  // Combina password e secondo fattore in una stringa e usa PBKDF2
  const combined = password + ':' + secondFactorSecret;
  const enc = new TextEncoder();
  const keyMaterial = await crypto.subtle.importKey(
    "raw", enc.encode(combined), { name: "PBKDF2" }, false, ["deriveBits", "deriveKey"]
  );
  const derivedSalt = salt || crypto.getRandomValues(new Uint8Array(16));
  const derivedKey = await crypto.subtle.deriveKey(
    { name: "PBKDF2", salt: derivedSalt, iterations: 600000, hash: "SHA-256" },
    keyMaterial, { name: "AES-GCM", length: 256 }, true, ["encrypt", "decrypt"]
  );
  // Estraiamo la chiave raw per poterla usare in pack/unpack (ad esempio per vaultKeyRaw)
  const rawKey = await crypto.subtle.exportKey("raw", derivedKey);
  return { vaultKeyRaw: new Uint8Array(rawKey), salt: derivedSalt };
}